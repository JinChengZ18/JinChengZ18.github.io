...
;(function($) {
    ...
    if ($.isFunction(options)) {
		...
		// VARIABLES	
        var $popup 			= this
          , d 				= options.document || $(document)
          , w 				= options.window || window
		  , $w				= $(w)
          , wH				= windowHeight()
		  , wW				= windowWidth()
          , prefix			= '__b-popup'
...
//					iframe.appendTo(o.contentContainer);   renzhs 注释于2014.03.23，防止弹出窗口加载多个iframe

					var iframe = o.contentContainer.find('iframe');
					if(iframe.length<1){
						iframe=$('<iframe class="b-iframe" ' + o.iframeAttr +'></iframe>');
        ...
        function close() {
            if (o.modal) {
                $(o.appendTo).find('.b-modal.'+$popup.data('id'))
		...
		function triggerCall(func) {
       	...
       	function calPosition(){
       		...
       		if(o.outerWidth)
				...
				outerWidth = $popup.outerWidth(true);
			if(o.outerHeight)
				outerHeight =o.outerHeight;
			else
				outerHeight = $popup.outerHeight(true);
			
			vPos 		= fixedVPos ? o.position[1] : Math.max(0, ((wH- outerHeight) / 2) - o.amsl)
			, hPos 		= fixedHPos ? o.position[0] : (wW - outerWidth) / 2
			, inside 	= insideWindow();
		};
        ...
        function insideWindow(){
            ...
            return wH >outerHeight && wW >outerWidth;
        };
		
		function windowHeight(){
			return window.self.innerHeight || $(window.self).height();
		};
		
		function windowWidth(){
			return window.self.innerWidth || $(window.self).width();
		};
    };
